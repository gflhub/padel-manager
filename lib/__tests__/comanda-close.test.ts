...
describe('closeComanda', () => {
  ...
  it('rejects closing without a payment method', async () => {
    const result = await closeComanda('comanda-1', 'club-a', '', 'user-1')

    expect(result.error).toBe('Método de pagamento é obrigatório')
    expect(comandaFindUnique).not.toHaveBeenCalled()
  })
  ...
  it('returns not-found when the comanda does not exist', async () => {
    ...
    const result = await closeComanda('missing-comanda', 'club-a', 'cash', 'user-1')
